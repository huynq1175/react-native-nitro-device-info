...
class ReactNativeNitroDeviceInfo: HybridReactNativeNitroDeviceInfoSpec {
  ...
  /// Device model marketing name (e.g., "iPhone 15 Pro", "iPad Air")
  public var model: String {
    let id = cachedDeviceModelIdentifier

    if let name = Self.deviceNamesByCode[id] {
      return name
    }

    // Fallback to generic name for unknown/future devices
    ...
    if id.hasPrefix("RealityDevice") { return "Apple Vision" }
  ...
  /// Hardware identifier to marketing name mapping
